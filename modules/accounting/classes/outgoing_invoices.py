...
import json
create_table_query = "CREATE TABLE "+classes_names.outgoing_invoices+"" \
...
def getById(connnection, cursor, id):
    try:
        ...
        query = " SELECT * from " + classes_names.outgoing_invoices + " where local_id=%s ;"
...
def getAll(connnection, cursor):
    try:
        ...
        query = " SELECT * from " + classes_names.outgoing_invoices + " ;"
...
def InsertInvoice(connnection,cursor,title,invoice_id,created_date,expiration_date,fullfillement_date
    ...
    try:
        query = "INSERT INTO "+classes_names.outgoing_invoices+" (title,invoice_id,created_date,expiration_date,fullfillement_date," \
                    "total_amount,total_vat,currency,products,status)" \
        ...
        print ("Exception during inserting into outgoing_invoices table , ERROR : ", error)



def GetAllOutgoingInvoices(connection,cursor):
    try:
        var=[]
        query=" SELECT invoice_id from "+classes_names.outgoing_invoices+" ;"
        cursor.execute (query)
        query_result = cursor.fetchall ()
        if (len(query_result)!=0):
            for x in range(len(query_result)):
...
def update (connection,cursor,invoice_id,balance):
    try:

        query = " SELECT status from " + classes_names.outgoing_invoices + " where invoice_id =%s ;"
        ...
        if (len(query_result)!=0):
            ...
            if(balance == 0.0 and query_result[0][0]!="PAID"):
                try:
                    ...
                    create_query = " UPDATE " + classes_names.outgoing_invoices + " set status = %s "\
                                                               " where invoice_id = (%s) ;"
